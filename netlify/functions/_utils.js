...
function getAuth0Config() {
  ...
  const rolesClaim = process.env.AUTH0_ROLES_CLAIM || 'https://claimsfox.com/roles'
  const issuer = issuerRaw.endsWith('/') ? issuerRaw : `${issuerRaw}/`

  if (!domain || !issuer || !audience) {
    throw new Error('AUTH0_DOMAIN, AUTH0_ISSUER and AUTH0_AUDIENCE must be configured')
  }
...
function verifyJwtSignature(parsed, jwk) {
  ...
  if (algorithm !== 'RS256') throw new Error(`Unsupported JWT alg: ${algorithm}`)
...
function validateStandardClaims(payload, issuer, audience) {
  ...
  if (!payload?.iss || payload.iss !== issuer) throw new Error('Invalid issuer')

  const aud = payload?.aud
  const audienceMatch = Array.isArray(aud) ? aud.includes(audience) : aud === audience
  if (!audienceMatch) throw new Error('Invalid audience')
...
function extractRoles(payload, rolesClaim) {
  ...
  const fallback = payload?.app_metadata?.roles
  return sanitizeRoles(claimRoles || topRoles || fallback || [])
}
...
async function verifyAccessToken(token) {
  ...
  const signatureOk = verifyJwtSignature(parsed, key)
...
async function getAuth0ManagementToken() {
  ...
  const audience = process.env.AUTH0_MGMT_AUDIENCE || (domain ? `https://${domain}/api/v2/` : '')
  ...
  const payload = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(payload?.error_description || payload?.error || `Management token failed (${res.status})`)
  }
...
async function auth0ManagementRequest({ method = 'GET', path, body }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Auth0 Management API failed (${res.status}) ${msg.slice(0, 200)}`)
  }
